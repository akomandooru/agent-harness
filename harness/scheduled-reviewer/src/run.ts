...
import {
  ...
  type ContentBlock,
} from "../../../app/orchestrator/multi-turn-executor";

import { MapToolCatalogue, type ToolCatalogue } from "../../../app/orchestrator/tool-executor";
...
// ---------------------------------------------------------------------------
// ManagedHarnessReviewerInvocationOptions — multi-turn extension
// ---------------------------------------------------------------------------

/**
 * Construction options for `ManagedHarnessReviewerInvocation` with
 * multi-turn support.
 *
 * Extends the base `ReviewerHarnessClientOptions` with the tool catalogue
 * and max-turn cap required by `MultiTurnExecutor`.
 */
export interface ManagedHarnessReviewerInvocationOptions extends ReviewerHarnessClientOptions {
  /** Tool catalogue for inline_function execution during multi-turn loops. */
  ...
  implements StandaloneReviewerInvocation
{
  ...
  public constructor(options: ManagedHarnessReviewerInvocationOptions) {
  ...
  public async invoke(
    ...
    // Build the initial user message. The harness has its system prompt
    // and tool catalogue baked in; the user turn only carries the diff
    // the agent should review. In scheduled mode (no diff supplied), the
    // reviewer reads the module state directly via its module.readFile
    // tool, so we pass an empty payload.
    const userMessage = JSON.stringify({ diff: input?.diff ?? null });
    ...
    // Execute the multi-turn loop. The MultiTurnExecutor handles
    // stopReason: "tool_use" by executing inline_function tools and
    // re-invoking InvokeHarness until a terminal stop reason is received.
    // Requirements: 2.1, 2.2, 2.3
    const multiTurnResult: MultiTurnResult = await this.executor.execute(initialMessages);

    // Extract the assistant's final text content from the multi-turn result.
    const assistantText = this.extractFinalAssistantText(multiTurnResult.finalContent);

    // Compute tokenCostUSD. The multi-turn executor does not currently
    // surface per-turn metadata/token usage, so default to 0. A future
    // enhancement can accumulate token usage across turns.
    const tokenCostUSD = 0;

    // Parse the assistant's JSON output. Track the parse error
    ...
    // Extract findings, defaulting to [] when absent or malformed.
    const findings = extractFindings(parsed);

    // Extract modelVersion, defaulting when absent.
  ...
  /**
   * Extract the final assistant text from the multi-turn result's content blocks.
   * The reviewer's structured JSON output is expected to be in the last text
   * content block of the final assistant message.
   */
  private extractFinalAssistantText(finalContent: ContentBlock[]): string {
    ...
    // The reviewer's structured JSON output is the *last* text block.
...
function defaultReviewerInvocation(): StandaloneReviewerInvocation {
  ...
  // The scheduled reviewer runs in read-only mode (no inline_function tools
  // to execute locally). Provide an empty tool catalogue; multi-turn loops
  // will only continue if the harness requests tools, and the reviewer's
  // tools (module.readFile, module.diff, reference.checklist) are all
  // server-side (type: "server_tool_use"), not inline_function.
  const toolCatalogue = new MapToolCatalogue();

  return new ManagedHarnessReviewerInvocation({ harnessArn, sessionId, toolCatalogue });
}
