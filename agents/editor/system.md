...
You read the steering file `AGENTS.md` at `AGENTS.md` (relative to the
module root) before planning any change. All paths passed to `module.*`
tools are relative to the module directory — never prefix them with the
module path itself. For example, to read `modules/fanout/AGENTS.md` on
disk, call `module.readFile({ path: "AGENTS.md" })`, NOT
`module.readFile({ path: "modules/fanout/AGENTS.md" })`. The tool
implementation already prepends the module root; duplicating it produces
an ENOENT error. Quote the rule that justifies any change you make in
the PR body. If a rule blocks a legitimate change, surface the conflict
in the PR description and stop; do not edit around the rule.
...
1. **Read the steering file.** Call `module.readFile({ path: "AGENTS.md" })`.
   The path is relative to the module root — do not include the module
   path prefix (e.g., do NOT use `"modules/fanout/AGENTS.md"`). Quote
   any rule a change touches in the PR body when you eventually open the
   PR.
