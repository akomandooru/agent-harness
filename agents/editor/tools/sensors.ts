...
/**
 * Default sensor runner. On Windows uses `shell: true` to resolve `.cmd`
 * shims. On Linux/Lambda invokes the underlying JS entry of each tool
 * directly via the running node binary (process.execPath) — bypasses
 * shell, PATH, and shebang resolution which all fail under Lambda's
 * restricted runtime.
 */
export const defaultSensorRunner: SensorRunner = {
  async run(
    ...
    return new Promise<SensorRunResult>((resolveRun, rejectRun) => {
      ...
      const isLinux = !useShell;
      let resolvedCommand = command;
      let resolvedArgs: readonly string[] = args;
      if (isLinux && command === "npx") {
        // Strip "npx" + tool-name; invoke the tool's JS entry via node.
        // args[0] is the tool name (cdk, tsc, eslint, jest); the rest
        // are the tool's own arguments.
        const toolName = args[0];
        const toolArgs = args.slice(1);
        const toolJsPath: Record<string, string> = {
          cdk: "/var/lang/lib/node_modules/aws-cdk/bin/cdk",
          tsc: "/var/lang/lib/node_modules/typescript/bin/tsc",
          eslint: "/var/lang/lib/node_modules/eslint/bin/eslint.js",
          jest: "/var/lang/lib/node_modules/jest/bin/jest.js",
        };
        const jsEntry = toolName === undefined ? undefined : toolJsPath[toolName];
        if (jsEntry !== undefined) {
          resolvedCommand = process.execPath;
          resolvedArgs = [jsEntry, ...toolArgs];
        }
      }
