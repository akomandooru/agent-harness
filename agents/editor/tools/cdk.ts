...
export const defaultCdkRunner: CdkRunner = {
  async run(
    ...
    return new Promise<CdkRunResult>((resolveRun, rejectRun) => {
      // On Windows, shell:true so npx.cmd resolves via PATH.
      // On Linux/Lambda, spawn node directly with the cdk JS entry.
      // process.execPath is the path of the currently-running node, which
      // is the only path guaranteed to work in Lambda's overlay runtime.
      ...
      if (isLinux) {
        ...
        // The global cdk install lives at /var/lang/lib/node_modules/aws-cdk/bin/cdk
        // — a JS file. Pass it as the first arg to node.
        const cdkJs = "/var/lang/lib/node_modules/aws-cdk/bin/cdk";
        command = nodeBinary;
        spawnArgs = [cdkJs, ...args];
      } else {
        command = "npx";
        spawnArgs = ["cdk", ...args];
      }
