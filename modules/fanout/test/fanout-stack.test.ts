...
describe("SQS", () => {
  test("two queues, both KMS-encrypted with the same key", () => {
    template.resourceCountIs("AWS::SQS::Queue", 2);
    const queues = template.findResources("AWS::SQS::Queue");
    const masterKeys = new Set<string>();
    for (const queue of Object.values(queues)) {
      expect(queue.Properties.KmsMasterKeyId).toBeDefined();
      masterKeys.add(JSON.stringify(queue.Properties.KmsMasterKeyId));
    }
    // Both queues reference the same key (same intrinsic).
    expect(masterKeys.size).toBe(1);
  });
  ...
  test("both queues have policies denying non-HTTPS access", () => {
...
describe("Lambda", () => {
  test("two functions, both Node 20.x ARM64 with reserved concurrency 5", () => {
