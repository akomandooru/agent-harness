...
export class FanoutStack extends Stack {
  ...
  private buildDeadLetterQueue(key: kms.Key): sqs.Queue {
  ...
  private buildQueue(key: kms.Key, dlq: sqs.Queue): sqs.Queue {
  ...
  private buildEgressFn(queue: sqs.Queue, key: kms.Key): lambda.Function {
